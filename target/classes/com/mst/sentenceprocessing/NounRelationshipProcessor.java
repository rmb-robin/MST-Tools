...
public class NounRelationshipProcessor extends RelationshipProcessorBase implements RelationshipProcessor {
    ...
    private List<TokenRelationship> processSingleToken(WordToken wordToken) {
        ...
        if (index > 0) {
            ...
            if (nounRelationship != null)
                tokenRelationships.add(createRelationshipAndAnnotateWordTokens(nounRelationship.getEdgeName(), wordToken.getDescriptor(), wordTokens.get(index - 1), wordToken));
        }
    ...
    private List<TokenRelationship> iterateMap(Map<String, List<RelationshipMapping>> map, String key, WordToken wordToken) {
        ...
        if (!map.containsKey(key))
            return result;
        String descriptor = wordToken.getDescriptor();
        int startIndex = wordTokens.indexOf(wordToken);
        for (RelationshipMapping relationship : map.get(key)) {
            List<TokenRelationship> collection = processSingleNounRelationship(relationship, descriptor, startIndex);
            result.addAll(collection);
        }
        return result;
    }

    private List<TokenRelationship> processSingleNounRelationship(RelationshipMapping relationshipMapping, String descriptor, int startIndex) {
        List<TokenRelationship> result = new ArrayList<>();
        if (relationshipMapping.getIsToWildcard()) {
            result.add(createRelationshipAndAnnotateWordTokens(relationshipMapping.getEdgeName(), descriptor, wordTokens.get(startIndex), wordTokens.get(startIndex + 1)));
            return result;
        }
        int maxDistance = 7;
        ...
        for (int i = startIndex + 1; i <= endIndex; i++) {
            ...
            if (isWordTokenMatchToRelationship(relationshipMapping.getIsToSemanticType(), false, relationshipMapping.getToToken(), toToken))
                result.add(createRelationshipAndAnnotateWordTokens(relationshipMapping.getEdgeName(), descriptor, wordTokens.get(startIndex), wordTokens.get(i)));
        }
        return result;
    }

    private TokenRelationship createRelationshipAndAnnotateWordTokens(String edgeName, String descriptor, WordToken fromToken, WordToken toToken) {
        return tokenRelationshipFactory.create(edgeName, descriptor, EdgeTypes.related, fromToken, toToken, this.getClass().getName());
    }
