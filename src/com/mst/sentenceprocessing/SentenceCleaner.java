...
public class SentenceCleaner {
	...
	public SentenceCleaner() {
		...
		String sb = "p(-interaction)?\\s*[<>=]+\\s*%1$s\\s*(x|\\xd7|\\xc3\\x97)?(\\s*%1$s\\(%1$s\\))?" + // P-Value
				"|" +
				"%1$s\\s*(confidence interval)?\\s*([(\\[]CI[)\\]]|CI)\\s*[;,=]?\\s*%1$s\\s*(to|-)\\s*%1$s" + // Confidence Interval
				"|" +
				"(hazard ratio)?\\s*([(\\[]HR[)\\]]|HR)\\s*%1$s" + // HR
				"|" +
				"(risk ratio)?\\s*([(\\[]RR[)\\]]|RR)\\s*[=]?\\s*%1$s" + // Risk Ratio
				"|" +
				"(\\s*[\\w\\s]+\\s+)?n\\s*[<=]+\\s*%1$s" + // Number of Patients
				"|" +
				"%1$s\\s*patient[\\s\\-]years?" + // Patient-Years
				"|" +
				"range\\s*%1$s\\s*(-|to)\\s*%1$s";
		String completePattern = String.format(sb // Range TODO: both numbers need to have optional dosage matchers added afterwards
				, numberPattern);
	...
	private String processAge(String sentence) {
		try {
			...
			String YEAR_OLD_REPL = "-year-old";
			sentence = YEAR_OLD_REGEX.matcher(sentence).replaceAll(YEAR_OLD_REPL);
			String YEAR_OLD_SEX_REPL = "-year-old male";
	...
	private String processMeasurements(String sentence) {
		try {
			...
			Matcher measu = Constants.MULTIDIMENSIONAL_MEASUREMENT_REGEX.matcher(sentence);
			while(measu.find()) {
