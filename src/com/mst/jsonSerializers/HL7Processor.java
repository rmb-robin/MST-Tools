...
public class HL7Processor {
    ...
    SentenceTextRequest processMessage() {
    ...
    Map<String, String> getAllAvailableHl7Fields(AllHl7Elements allelements) {
    ...
    private String getHL7Value(HL7Element element) {
        String value = null;
        try {
            if (element.getLocation().contains("OBX-5")) {
    ...
    private boolean valueIsNull(String value) {
        return value == null || value.trim().length() == 0 || value.toUpperCase().startsWith("UNK");
    }
}
