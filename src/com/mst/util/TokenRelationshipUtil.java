...
public class TokenRelationshipUtil {
        ...
        for (TokenRelationship tokenRelationship : relationships) {
            if (!isNamed || tokenRelationship.getEdgeName().equals(EdgeNames.existence)) {
                ...
                if (!result.containsKey(tokenRelationship.getNamedEdge()))
                    result.put(tokenRelationship.getNamedEdge(), new ArrayList<>());
    ...
    public static boolean isEdgeMatchFromHas(TokenRelationship tokenRelationship, HashSet<String> edgeNameHash) {
        if (tokenRelationship.getNamedEdge() != null && edgeNameHash.contains(tokenRelationship.getNamedEdge()))
            return true;
        else if (tokenRelationship.getEdgeName() != null)
            return edgeNameHash.contains(tokenRelationship.getEdgeName());
        return false;
    }
}
