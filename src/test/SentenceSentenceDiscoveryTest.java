...
public class SentenceSentenceDiscoveryTest {
	...
	public void RunIterationRule() throws Exception {
		...
		List<TokenRelationship> r = RecommandedTokenRelationshipUtil
				.getTokenRelationshipsFromRecommendedTokenRelationships(discoveries.get(0).getWordEmbeddings());
	...
	private List<SentenceDb> processAndGetSentences(SentenceTextRequest request, String ip, String dbIp,
			boolean needInsert) throws Exception {
		if (needInsert) {
			String endPoint = ip;
			ObjectMapper mapper = new ObjectMapper();
	...
	private String createFullPath() {
		return System.getProperty("user.dir") + File.separator + "testData" + File.separator
				+ "Sentence_SentenceDiscoveryTesting" + File.separator + "sentences.txt";
	}

	private String createOutputPath(String fileName) {
		return System.getProperty("user.dir") + File.separator + "testData" + File.separator
				+ "Sentence_SentenceDiscoveryTesting" + File.separator + fileName;
	}
	...
	private void assertSummary(List<Sentence> sentences, List<SentenceDiscovery> discoveries) throws Exception {
		...
		for (int i = 0; i < sentences.size(); i++) {
			...
			List<TokenRelationship> discoveryTokenRelationships = RecommandedTokenRelationshipUtil
					.getTokenRelationshipsFromRecommendedTokenRelationships(discovery.getWordEmbeddings());

			Map<String, List<TokenRelationship>> sentenceEdges = TokenRelationshipUtil
					.getMapByEdgeName(sentence.getTokenRelationships(), false);
			Map<String, List<TokenRelationship>> discoveryEdges = TokenRelationshipUtil
					.getMapByEdgeName(discoveryTokenRelationships, true);
		...
		for (Entry<String, sentenceEdgeCompare> entry : edgesByMatchingCount.entrySet()) {
			...
			sbAppendWithLineBreak("Discovery Contains All Sentence Edges:  " + entry.getValue().discoveryContainsAll,
					sb);
			sb.append(System.getProperty("line.separator"));
			sb.append(System.getProperty("line.separator"));
		}
	...
	private void assertSummaryToMongo(List<Sentence> sentences, List<SentenceDiscovery> discoveries) throws Exception {
		...
		for (int i = 0; i < sentences.size(); i++) {
			Sentence sentence = sentences.get(i);
			SentenceDiscovery discovery = discoveries.get(i);
			SingleSentenceCompare sentenceCompare = createSingleSentenceCompare(sentence.getNormalizedSentence(),
					discovery.getNormalizedSentence());
			...
			Map<String, List<TokenRelationship>> sentenceEdgesByEdgeName = TokenRelationshipUtil
					.getMapByEdgeName(sentence.getTokenRelationships(), false);
			Map<String, List<TokenRelationship>> discoveryEdgesByEdgeName = TokenRelationshipUtil
					.getMapByEdgeName(discoveryTokenRelationships, true);
			...
			for (Entry<String, List<TokenRelationship>> entry : sentenceEdgesByEdgeName.entrySet()) {
				...
				if (!discoveryEdgesByEdgeName.containsKey(entry.getKey())) {
					SentenceEdgeCompare edgeCompare = createSentenceEdgeCompare(entry.getKey(), sentenceToFroms.size(),
							0, sentenceToFroms, null);
				...
				SentenceEdgeCompare edgeCompare = createSentenceEdgeCompare(entry.getKey(), sentenceToFroms.size(),
						tofroms.size(), sentenceToFroms, tofroms);
	...
	private SentenceEdgeCompare createSentenceEdgeCompare(String edgeName, int sentenceAOccurance,
			int sentenceBOcurrance, List<String> sentenceAToFrom, List<String> sentenceBtoFrom) {
	...
	private void assertEdges(List<SentenceDb> old, List<SentenceDb> newSentences, boolean useSecondAsDiscovery)
			throws Exception {
		...
		for (int i = 0; i < old.size(); i++) {
			...
			if (TokenRelationshipComparer.areCollectionsSame(sentence.getTokenRelationships(),
					discovery.getTokenRelationships(), useSecondAsDiscovery)) {
				sb.append(System.getProperty("line.separator"));
				sb.append("Edges Match");
				appendEndOfSentenceToFile(sb);
				continue;
